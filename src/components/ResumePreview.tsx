...
function PersonalInfoHeader({ resumeData }: ResumeSectionProps) {
  ...
  useEffect(() => {
    const objctUrl = photo instanceof File ? URL.createObjectURL(photo) : ""
    if (objctUrl) setPhotoSrc(objctUrl)
    if (photo === null) setPhotoSrc("")

    return () => URL.revokeObjectURL(objctUrl)
  }, [photo])

  return (
    <div className="flex items-center gap-6 ">
      {photoSrc && (
        <Image src={photoSrc} width={100} height={100} alt="Author photo" className="aspect-square object-cover"   style={{
          borderRadius:
